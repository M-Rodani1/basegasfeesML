#!/usr/bin/env python3
"""
Train model predicting log(gas_price) for better stability
...
def remove_outliers_robust(X, y):
    """Remove outliers using robust IQR method - less aggressive for log scale"""
    q1 = y.quantile(0.25)
    q3 = y.quantile(0.75)
    iqr = q3 - q1
    # Use 3 IQR for log scale (less aggressive)
    lower_bound = q1 - 3 * iqr
    upper_bound = q3 + 3 * iqr
    # For log scale, don't apply min/max multipliers
    mask = (y >= lower_bound) & (y <= upper_bound)
    removed = (~mask).sum()
    if removed > 0:
        print(f"Removed {removed} outliers ({removed/len(y)*100:.1f}%)")
        if removed > len(y) * 0.5:
            print(f"⚠️ WARNING: Removed >50% of data, skipping outlier removal")
            return X, y
    return X[mask], y[mask]


def train_log_model():
    """Train model predicting log(gas_price) for stability"""
    
    print("="*70)
    print("TRAINING LOG-SCALE MODEL (Target: R² > 70%)")
    print("="*70)
    
    # 1. Load data
    print("\n[1/6] Loading data...")
    db = DatabaseManager()
    historical = db.get_historical_data(hours=720)
    ...
    # 3. Create target variables - use LOG scale for stability
    print("\n[3/6] Creating log-scale target variables...")
    ...
    # Use log scale for target (more stable)
    log_gas = np.log(original_gas + 1e-8)  # Add small epsilon to avoid log(0)
    
    # Create 1h target (shift forward by 12 steps)
    y_1h_raw = pd.Series(log_gas).shift(-12)
    
    valid = ~y_1h_raw.isna()
    X_1h = X_full[valid].reset_index(drop=True)
    y_1h_log = y_1h_raw[valid].reset_index(drop=True)
    y_1h_actual = pd.Series(original_gas)[valid].reset_index(drop=True)  # Keep actual for evaluation
    
    print(f"✅ 1h targets: {len(y_1h_log)} samples")
    print(f"   Log range: {y_1h_log.min():.6f} to {y_1h_log.max():.6f}")
    
    # 4. Remove outliers
    print("\n[4/6] Removing outliers...")
    X_clean, y_clean_log = remove_outliers_robust(X_1h, y_1h_log)
    
    # Also filter actual values
    y_clean_actual = y_1h_actual[X_clean.index].reset_index(drop=True)
    ...
    y_train_log, y_test_log = y_clean_log.iloc[:split_idx], y_clean_log.iloc[split_idx:]
    y_train_actual, y_test_actual = y_clean_actual.iloc[:split_idx], y_clean_actual.iloc[split_idx:]
    
    # Use RobustScaler for features
    feature_scaler = RobustScaler()
    X_train_scaled = feature_scaler.fit_transform(X_train)
    X_test_scaled = feature_scaler.transform(X_test)
    
    # Scale log target (though log is already more stable)
    target_scaler = StandardScaler()
    y_train_scaled = target_scaler.fit_transform(y_train_log.values.reshape(-1, 1)).ravel()
    y_test_scaled = target_scaler.transform(y_test_log.values.reshape(-1, 1)).ravel()
    ...
    for name, model in models_to_try.items():
        ...
        y_pred_log = target_scaler.inverse_transform(y_pred_log_scaled.reshape(-1, 1)).ravel()
        
        # Convert back to actual gas prices
        y_pred = np.exp(y_pred_log) - 1e-8  # Inverse log
        ...
        if len(y_test_actual) > 1:
            y_test_diff = np.diff(y_test_actual.values)
    ...
    save_data = {
        ...
        'uses_log_scale': True,  # Flag that we use log scale
